...
namespace SoulTraders.Core
{
    ...
    /// </summary>
    public static partial class STEvents
    {

        static HeapQueue<STEvent> eventQueue = new HeapQueue<STEvent>();
        static Dictionary<System.Type, Stack<STEvent>> eventPools = new Dictionary<System.Type, Stack<STEvent>>();
        ...
        /// <returns></returns>
        static public T New<T>() where T : STEvent, new()
        {
            Stack<STEvent> pool;
            if (!eventPools.TryGetValue(typeof(T), out pool))
            {
                pool = new Stack<STEvent>(4);
                pool.Push(new T());
        ...
        static public T Schedule<T>(float tick = 0) where T : STEvent, new()
        {
            var ev = New<T>();
        ...
        /// <param name="tick">Tick.</param>
        /// <typeparam name="T">The event type parameter.</typeparam>
        static public T Reschedule<T>(T ev, float tick) where T : STEvent, new()
        {
            ev.tick = Time.time + tick;
            eventQueue.Push(ev);
            return ev;
